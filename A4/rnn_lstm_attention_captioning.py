...
def rnn_step_backward(dnext_h, cache):
    ...
    # Trig identity of sech^2 = 1-tanh^2
    # print(Wx.shape)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(x.shape[1]):
      # print(i)
      if i == 0:
        h[:, i, :], cache_tick = rnn_step_forward(x[:, i, :], h0.clone(), Wx, Wh, b)
        # print(i, h)
      else:
        h[:, i, :], cache_tick = rnn_step_forward(x[:, i, :], h[:, i-1, :].clone(), Wx, Wh, b)
        # print(i, h)

      cache.append(cache_tick)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return h, cache
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    # Reference: cache = (x, prev_h, Wx, Wh, next_h)
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    # loss = torch.mean(torch.sum(F.cross_entropy(x.squeeze(), y.squeeze(), ignore_index=ignore_index, reduction='mean')))
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        ############################################################################
        # Replace "pass" statement with your code
        # Referencing "Captioning Module" cell in notebook
        # Instantiate WordEmbedding class to create property with parameter of shape [3, 30]
        #    3 = How many words present in the vocab dictionary
        #    30 = wordvec_dim
        # Call forward method of instantiation with 'captions_in' of shape [10, 12]
        #    10 = Minibatch size (N)
        #    12 = Sequence Length (T)
        #    Each element in captions_in ranges from [0, 3)
        # Returns output x of shape [10, 12, 30]

        features = self.featureExtractor.extract_mobilenet_feature(images)
        # Indexing a [3, 30] tensor with a [10, 12] tensor to produce a [10, 12, 30]
        x = self.wordEmbedding_transform.forward(captions_in)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b, attn=None, Wattn=None):
    ...
    _i = actVector[:, 0:x]
    _f = actVector[:, x:2*x]
    _o = actVector[:, 2*x:3*x]
    _g = actVector[:, 3*x:4*x]
...
class LSTM(nn.Module):
  ...
  def __init__(self, input_size, hidden_size, device='cpu',
    ...
    # Register parameters
    # self.Wx = Parameter(torch.normal(torch.zeros(input_size, hidden_size*4), 1))
