...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  conv_w1 = nn.init.kaiming_normal_(torch.empty(channel_1, 3, kernel_size_1, kernel_size_1, dtype=dtype, device=device))
  ...
  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes, 32*32*16, dtype=dtype, device=device))
...
def initialize_three_layer_conv_part4():
  ...
  model = nn.Sequential(OrderedDict([
    ('conv1', nn.Conv2d(C, channel_1, kernel_size_1, bias=True, padding=2)),
    ('relu1', nn.ReLU()),
    ('conv2', nn.Conv2d(channel_1, channel_2, kernel_size_2, bias=True, padding=1)),
    ('relu2', nn.ReLU()),
    ('flatten', nn.Flatten()),
    ('fc', nn.Linear(32*32*channel_2, num_classes, bias=True))
  ]))
...
class ResNet(nn.Module):
  ...
  def forward(self, x):
    ...
    scores = self.cnn(x)
    # print(scores.shape)
    avgPool = nn.AvgPool2d(scores.shape[2])
    scores = avgPool(scores)
    # print(scores.shape)
