...
class ProposalModule(nn.Module):
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    anchor_features = self.pred_layer(features) # (B, A*6, 7, 7)

    # Segregate anchors & anchor elements
    B, _, H, W = anchor_features.shape
    anchor_features = anchor_features.reshape(B, self.num_anchors, 6, H, W)

    # Segregate confidence scores & offsets 
    conf_package = anchor_features[:, :, :2, :, :]
    offsets_package = anchor_features[:, :, 2:, :, :]

    if mode == 'eval':
      # Simply return predicted values if evaluating
      conf_scores, offsets = conf_package, offsets_package
    else:
      # Find associated values for GT positive anchor indices 
      pos_conf = self._extract_anchor_data(conf_package, pos_anchor_idx)
      # Find associated values for GT negative anchor indices 
      neg_conf = self._extract_anchor_data(conf_package, neg_anchor_idx)

      # Combine because that's how it's asked to be returned
      conf_scores = torch.cat((pos_conf, neg_conf))

      # Find associated offsets for GT positive anchor indices
      offsets = self._extract_anchor_data(offsets_package,pos_anchor_idx)

      # Apply transformations using GenerateProposal()
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
    ...
    activated_anc_ind, negative_anc_ind, GT_conf_scores, GT_offsets, GT_class, \
      activated_anc_coord, _ = ReferenceOnActivatedAnchors(anc_list, bboxes, grid_list, iou_mat, method='FasterRCNN')

    conf_scores, offsets, proposals = self.prop_module(features, activated_anc_coord, activated_anc_ind,
      negative_anc_ind)
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    mask = conf_scores > thresh

    # anc_list = anc_list.permute(0, 1, 4, 2, 3)
    offsets = offsets.permute(0, 1, 3, 4, 2)
    proposals = GenerateProposal(anc_list, offsets)
    
    with torch.no_grad():
      for i in range(images.shape[0]):
        sub_prop = proposals[i][mask[i]]
        sub_conf = conf_scores[i][mask[i]]
        
        nmsMask = nms(sub_prop, sub_conf, iou_threshold=nms_thresh)
        idx = nmsMask.long()


        final_proposals.append(sub_prop[idx])
        final_conf_probs.append(sub_conf[idx].unsqueeze(1))
