...
def prepocess_input_sequence(
    ...
    #     if i.isnumeric():
    #         [out.append(token_dict[k]) for k in i]            
    #     else:
    #         out.append(token_dict[i])

    tokens = input_str.split()
    for token in tokens:
        if token in spc_tokens:   # if token is a special token
            out.append(token_dict[token])
        else:   # if token is a list of digit
            for char in token:
...
def scaled_dot_product_two_loop_single(
    ...
    for k in range(K):
        for q in range(K):
            # One query dotting every key = a row
            e[k, q] = query[k].dot(key[q]) / M**(1/2) # torch.sqrt(torch.tensor(M))

        # Softmax along the dimension of each query dotting every key
        # I am pretty sure this is how it's desired to be done here - everything operating in the loop.
        a = e[k].softmax(0)
        # out[k] = torch.matmul(value.t(), a)
        out[k] = torch.matmul(a, value)

    # A better way that batches everything to probably work faster.
    # a = e.softmax(dim=1)
    # print('a', a.shape)
...
def scaled_dot_product_two_loop_batch(
    ...
    # Batch compute similarity matrix
    perm_key = key.permute(0, 2, 1)
    e = torch.bmm(query, perm_key)

    # Iterate over kth batch
    for k in range(N):
        e[k] = (e[k] / M**(1/2)).softmax(dim=1)
        
        # After preprocessing kth batch of similarity, matmul with kth value matrix
        for z in range(K):
            # out[k, z] = torch.matmul(value[k].t(), e[k, z])
            out[k, z] = torch.matmul(e[k, z], value[k])
        # out[k] = torch.matmul(e[k], value[k])
...
def scaled_dot_product_no_loop_batch(
    ...
    if mask is not None:
        ...
        # -1e9 = -1 * 10^9 = -1000000000.0

        # e[mask] = -1e9
        # print('q', query.shape, 'k', key.shape)
        # print('dot mask', mask.shape)
        # print('dot e', e.shape)
        e = e.masked_fill_(mask.to(e.device), -1e9)

    # y = torch.matmul(a, value) # also works
    weights_softmax = (e / M**(1/2)).softmax(dim=-1)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        self.c2 = (6/(dim_in + dim_v))**(1/2)

        self.q = nn.Linear(dim_in, dim_q)
        torch.nn.init.uniform_(self.q.weight, -self.c, self.c)

        self.k = nn.Linear(dim_in, dim_q)
        torch.nn.init.uniform_(self.q.weight, -self.c, self.c)

        self.v = nn.Linear(dim_in, dim_v)
        torch.nn.init.uniform_(self.q.weight, -self.c2, self.c2)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code 
        # Typo in the TODO? says `self_attention_no_loop_batch`, but am confident it means
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # print('wtf', dim_in // num_heads)

        self.c = (6/(dim_out + dim_in))**(1/2)
        self.heads = torch.nn.ModuleList([SelfAttention(dim_in, dim_out, dim_out) for i in range(num_heads)])
        self.linear = nn.Linear(dim_out * num_heads, dim_in)
        torch.nn.init.uniform_(self.linear.weight, -self.c, self.c)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
    ...
    def forward(
        ...
        output_list = []
        for m in self.heads:
            y = m(query, key, value, mask)
            output_list.append(y)
        # Make sure to concat to trailing dim to ensure operation on different dim inputs
        concat = torch.cat(output_list, dim=-1)
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        if (len(x.shape) == 2):
            mean = x.mean(dim=-1).unsqueeze(0).transpose(-1, -2).repeat(1, x.shape[1])
            var = x.var(dim=-1, unbiased=False).unsqueeze(0).transpose(-1, -2).repeat(1, x.shape[1]) # Bessel's correction is default true... caused errors
        
        if (len(x.shape) > 2):
            mean = x.mean(dim=-1).unsqueeze(2).repeat(1, 1, x.shape[1])
            var = x.var(dim=-1, unbiased=False).unsqueeze(2).repeat(1, 1, x.shape[1]) # Bessel's correction is default true... caused errors


        # Computing var w/out using torch.var() because I didn't know Bessel's was default on.
        # var = torch.mean(torch.square(x - mean), dim=-1)
        normalized = (x - mean) / (var + self.epsilon)**(1/2)
        y = normalized * self.gamma + self.beta
        # for i in range(x.shape[0]):
        #     y[i] = ((x[i] - mean[i]) / torch.sqrt(var[i] + self.epsilon)) * self.gamma + self.beta
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.c = (6/(inp_dim + hidden_dim_feedforward))**(1/2)
        ...
        self.mlp[0].weight = torch.nn.init.uniform_(self.mlp[0].weight, -self.c, self.c)
        self.mlp[2].weight = torch.nn.init.uniform_(self.mlp[2].weight, -self.c, self.c)

        # Alternative way w/out using nn.Sequential()
        # self.linear1 = nn.Linear(inp_dim, hidden_dim_feedforward)
        # self.linear1.weight = torch.nn.init.uniform_(self.linear1.weight, -self.c, self.c)

        # self.act = nn.ReLU()

        # self.linear2 = nn.Linear(hidden_dim_feedforward, inp_dim)
        # self.linear2.weight = torch.nn.init.uniform_(self.linear2.weight, -self.c, self.c)
    ...
    def forward(self, x):
        ...
        # Replace "pass" statement with your code
        y = self.mlp(x)
        # Correspondence to the alternative way defined in __init__()
        # y = self.linear1(x)
        # y = self.act(y)
        # y = self.linear2(y)
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        # print('multihead dim_out: ', emb_dim // num_heads)
    ...
    def forward(self, x):
        ...
        y = self.layernorm(self.MultiHeadBlock(x, x, x) + x)
        y = self.dropout1(y)
...
def get_subsequent_mask(seq):
    ...
    # Can use torch.triu as done in nn.Transformer code. Make sure to convert to BOOLEAN
    # https://pytorch.org/docs/stable/_modules/torch/nn/modules/transformer.html#Transformer.forward
    mask = torch.triu(torch.full((seq.shape[1], seq.shape[1]), float('-inf'), dtype=torch.bool), diagonal=1)
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        As shown in the Figure 1 of the paper attention is all you need
        https://arxiv.org/pdf/1706.03762.pdf, the encoder [decoder (ryan)] consists of 5 components:   
        ...
        self.norm1 = nn.LayerNorm(emb_dim, eps=1e-10)
        # self.norm2 = nn.LayerNorm(emb_dim, eps=1e-10)
        # self.norm3 = nn.LayerNorm(emb_dim, eps=1e-10)
    ...
    def forward(
        ...
        y = self.norm1(self.attention_self(dec_inp, dec_inp, dec_inp, mask) + dec_inp)
        y = self.dropout1(y)

        y = self.norm1(self.attention_cross(y, enc_inp, enc_inp) + y)
        y = self.dropout1(y)

        y = self.norm1(self.feed_forward(y) + y)
        y = self.dropout1(y)
        # residual = dec_inp
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    # # p, i = 1, 0
    # # a = torch.floor(torch.tensor(2*i / M))
    # # sin = torch.sin(p / 10000**a.clone().detach())
    # # cos = torch.cos(p / 10000**a.clone().detach())
    ...
    #     # for i in range(M):
    #     a = torch.floor(torch.tensor(2*i / M))
    #     y[:, p, ::2] =  torch.sin(p / 10000**a.clone().detach())
    #     y[:, p, 1::2] = torch.cos(p / (10000**a.clone().detach()))
    # # print(y)
    y = torch.zeros(K, M)

    p = torch.arange(0, K).unsqueeze(1) # Make column vector
    i = torch.arange(0, M).unsqueeze(0) # Make row vector
    a = torch.floor(i / M) # 2*i?
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        args:
            ...
            ques_b and ans_b have different sequence lengths, hence K =/= K
        returns:
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        y = self.encoder(q_emb_inp)
        # test = torch.zeros(a_emb_inp.shape[0], a_emb_inp.shape[1])
        mask = get_subsequent_mask(ans_b[:, :-1])
        dec_out = self.decoder(a_emb_inp, y, mask)
...
class AddSubDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(preprocess_inp.shape, inp_pos_enc[0].shape, preprocess_out.shape, out_pos_enc[0].shape)
