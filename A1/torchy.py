...
def mutate_tensor(x, indices, values):
  ...
  x[indices[1]] = values[1]
  x[indices[2]] = values[2]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def create_tensor_of_pi(M, N):
  ...
  # Replace "pass" statement with your code
  pi = 3.14
  x = torch.full([M, N], pi) #Don't knonw what 'filled with sevens' means
...
def multiples_of_ten(start, stop):
  ...
  x = torch.tensor(g, dtype=torch.float64)
  # x = torch.arange(start, stop, start%10, dtype=torch.float64)
  # How the fuck do you use arange() for this
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def slice_assignment_practice(x):
  ...
  - You must use <= 6 slicing oprations to achieve the desired result
e
  Inputs:
  ...
  # x[0:2, 1:6] = torch.tensor([1, 2, 2, 2, 2])
...
def make_one_hot(x):
  ...
  # Better than the method commented above, doens't produce errors for Colab too...
  n = len(x)
  c = 1 + max(x)
  l = [range(n)]
...
def zero_row_min(x):
  ...
  print('waffles', torch.arange(x.shape[0]))
  y[torch.arange(x.shape[0]), torch.argmin(x, dim=1)] = 0

  # Highly unlikely to be the intended way to solve the problem.
  # if x.shape[0] == 2:
  #   x[0, x.argmin(1)[0]] = 0
  #   x[1, x.argmin(1)[1]] = 0
  # elif x.shape[0] == 3:
  #   x[0, x.argmin(dim=1)[0]] = 0
  #   x[1, x.argmin(dim=1)[1]] = 0
  #   x[2, x.argmin(dim=1)[2]] = 0
  # y = x
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
    ...
    # z = torch.zeros(2, 3, 4)
    z = torch.zeros_like(x.matmul(y))
    for i in torch.arange(x.shape[0]):
      z[i] = x[i].mm(y[i])

    # print('z1', z, z.shape)

    # z = torch.stack([z, z], 0) How to do with using torch.stack?
...
def normalize_columns(x):
  ...
  q = torch.zeros(1, x.shape[1])

  # Iterate over columns
  for i in torch.arange(x.shape[1]):
    # Mean
    z[0, i] = torch.sum(x[:,i]) / x.shape[0]
    # Std
    q[0, i] = torch.sqrt((torch.sum(pow(x[:, i] - z[0, i], 2)) / (x.shape[0] - 1))
)
  # print('z', z)
  # print('q', q)
  # print(x - z)
