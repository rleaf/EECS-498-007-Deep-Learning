...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    # print('hprime, wprime', hPrime, wPrime)
      ...
      for f in range(F):
        for h in range(0, hPrime):
          for W in range(0, wPrime):
            # CNN takes the sum of the elementwise multiplication between kernel and input of same dimensions
            # and then adds the bias associated with that filter.
            # Number of bias always corresponds 1:1 with number of filters
            # One bias per filter means b[i] sums to every summed element-wise between w[i] and xpad[i]
        ...
        for c in range(C):
            for j in range(hPrime):
                for i in range(wPrime):
                    ind = torch.argmax(x[n, c, j*stride:j*stride+pHeight, i*stride:i*stride+pWidth])
                    ind1, ind2 = ind//pHeight, ind%pWidth
                    dx[n, c, j*stride:j*stride+pHeight, i*stride:i*stride+pWidth][ind1, ind2] = dout[n, c, j, i]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    self.params['W1'] = torch.normal(torch.zeros(num_filters, C, filter_size, filter_size), weight_scale).to(dtype).to(device)
    self.params['b1'] = torch.zeros(num_filters).to(dtype).to(device)
    
    # Flattening n-dimensional tensor to make more ammenable for fully connected linear layer
    self.params['W2'] = torch.normal(torch.zeros(num_filters * H * W // 4, hidden_dim), weight_scale).to(dtype).to(device)
    self.params['b2'] = torch.zeros(hidden_dim).to(dtype).to(device)

    self.params['W3'] = torch.normal(torch.zeros(hidden_dim, num_classes), weight_scale).to(dtype).to(device)
  ...
  def loss(self, X, y=None):
    ...
    l2reg = self.reg * (torch.sum(torch.square(W1)) + torch.sum(torch.square(W2)) + torch.sum(torch.square(W3)))
    loss = smloss + l2reg

    dout, grads['W3'], grads['b3'] = Linear.backward(soft_max_grad, cache3)
    grads['W3'] += 2 * self.reg * grads['W3']
    dout, grads['W2'], grads['b2'] = Linear_ReLU.backward(dout, cache2)
    grads['W2'] += 2 * self.reg * grads['W2']
    _, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dout, cache1)
    grads['W1'] += 2 * self.reg * grads['W1']
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    # Conv layers
    layer_dims = num_filters + [num_classes]

    # Conv layers
    for i in range(len(num_filters)):
      if weight_scale == 'kaiming':
        if i == 0:
          # self.params['W%d' % (i + 1)] = torch.nn.init.kaiming_normal_(torch.zeros(layer_dims[i], C, 3, 3)).to(self.dtype).to(device)
          self.params['W%d' % (i + 1)] = kaiming_initializer(layer_dims[i], C, K=3, relu=True, device=device, dtype=dtype)
        else:
          # self.params['W%d' % (i + 1)] = torch.nn.init.kaiming_normal_(torch.zeros(layer_dims[i], layer_dims[i - 1], 3, 3)).to(self.dtype).to(device)
          self.params['W%d' % (i + 1)] = kaiming_initializer(layer_dims[i], layer_dims[i - 1], K=3, relu=True, device=device, dtype=dtype)
        self.params['b%d' % (i + 1)] = torch.zeros(layer_dims[i]).to(self.dtype).to(device)
      else:
        if i == 0:
          self.params['W%d' % (i + 1)] = torch.normal(torch.zeros(layer_dims[i], C, 3, 3), weight_scale).to(self.dtype).to(device)
        else:
          self.params['W%d' % (i + 1)] = torch.normal(torch.zeros(layer_dims[i], layer_dims[i - 1], 3, 3), weight_scale).to(self.dtype).to(device)
        self.params['b%d' % (i + 1)] = torch.zeros(layer_dims[i]).to(self.dtype).to(device)
      
      if self.batchnorm:
        self.params['gamma%d' % (i+1)] = torch.ones(layer_dims[i]).to(dtype).to(device)
        self.params['beta%d' % (i+1)] = torch.zeros(layer_dims[i]).to(dtype).to(device)      
    ...
    # Pooling
    # Each 2x2 max_pool with stride 2 divides the total element count by a factor of 4
    # 4^(number of times pooling is used)
    j = 4**len(self.max_pools)

    # Linear layer
    i+=1
    if weight_scale == 'kaiming':
      # self.params['W%d' % (i + 1)] = torch.nn.init.kaiming_normal_(torch.zeros(num_filters[i - 1] * H * W // j, num_classes)).to(self.dtype).to(device)
      self.params['W%d' % (i + 1)] = kaiming_initializer(num_filters[i - 1] * H * W // j, num_classes, device=device, dtype=dtype)
    else:
      self.params['W%d' % (i + 1)] = torch.normal(torch.zeros(num_filters[i - 1] * H * W // j, num_classes), weight_scale).to(self.dtype).to(device)
    self.params['b%d' % (i + 1)] = torch.zeros(layer_dims[i]).to(self.dtype).to(device)
  ...
  def loss(self, X, y=None):
    ...
    # Iterate first over the conv layers (3 conv layers)
    for i in range(self.num_layers - 1):
      ...
      scores, cache = FastConv.forward(scores, self.params['W%d' % (i + 1)], self.params['b%d' % (i + 1)], conv_param)
      total_cache.append(cache)

      if self.batchnorm:
        scores, cache = SpatialBatchNorm.forward(scores, self.params['gamma%d' % (i + 1)], self.params['beta%d' % (i + 1)], self.bn_params[i])
        total_cache.append(cache)
    ...
    ############################################################################
    # Replace "pass" statement with your code
    smloss, dx = softmax_loss(scores, y)
    l2reg = 0

    for i in range(self.num_layers):
      l2reg += torch.sum(torch.square(self.params['W%d' % (i + 1)]))
    ...
    while i >= 0:
      if i in self.max_pools:
        dx = FastMaxPool.backward(dx, total_cache.pop())
      dx = ReLU.backward(dx, total_cache.pop())
      if self.batchnorm:
        dx, grads['gamma%d' % (i + 1)], grads['beta%d' % (i + 1)] = SpatialBatchNorm.backward(dx, total_cache.pop())
      dx, grads['W%d' % (i+1)], grads['b%d' % (i+1)] = FastConv.backward(dx, total_cache.pop())
      grads['W%d' % (i+1)] += 2 * self.reg * self.params['W%d' % (i+1)]
      i -=1
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  model = DeepConvNet(input_dims=input_dims, num_classes=10,
                      # num_filters=[32, 32 ,64 ,64 ,128 ,12],
                      num_filters=[20, 40, 80],
                      max_pools=[0, 1, 2],
                      # max_pools=[9, 19],
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
  ...
  if K is None:
    ...
    # Replace "pass" statement with your code
    if relu:
      # weight = (gain/Din)**(1/2) * torch.rand(Din, Dout, dtype=dtype, device=device)
      weight = torch.normal(torch.zeros(Din, Dout, device=device,dtype=dtype), (2/Din)**(1/2))
      # weight = torch.normal(torch.zeros(Din, Dout, ), (2/Din)**(1/2)).to(device).to(dtype)
    else:
      # weight = (gain/Din)**(1/2) * torch.rand(Din, Dout, dtype=dtype, device=device)
      weight = torch.normal(torch.zeros(Din, Dout, device=device,dtype=dtype), (1/Din)**(1/2))
      # weight = torch.normal(torch.zeros(Din, Dout, ), (1/Din)**(1/2)).to(device).to(dtype)
    # if relu:
    ...
    # Replace "pass" statement with your code
    if relu:
      # weight = (gain/(Din * K * K))**(1/2) * torch.rand(Din, Dout, K, K, dtype=dtype, device=device)
      weight = torch.normal(torch.zeros(Din, Dout, K, K,device=device,dtype=dtype), (2/(Din * K * K))**(1/2))
      # weight = torch.normal(torch.zeros(Din, Dout, K, K,), (2/(Din * K * K))**(1/2)).to(device).to(dtype)
    else:
      # weight = (gain/(Din * K * K))**(1/2) * torch.rand(Din, Dout, K, K, dtype=dtype, device=device)
      weight = torch.normal(torch.zeros(Din, Dout, K, K,device=device,dtype=dtype), (1/(Din * K * K))**(1/2))
      # weight = torch.normal(torch.zeros(Din, Dout, K, K), (1/(Din * K * K))**(1/2)).to(device).to(dtype)
    # if relu:
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      #######################################################################
      # Replace "pass" statement with your code
      sMean = torch.mean(x, 0)
      # Set unbiased to false to account for Bessel's correction
      sVar = torch.var(x, 0, unbiased=False)

      running_mean = momentum * running_mean + (1 - momentum) * sMean
      running_var = momentum * running_var + (1 - momentum) * sVar

      # var = 1. / N * torch.sum((x - mu) ** 2, 0)
      xhat = (x - sMean)/torch.sqrt(sVar + eps)
      out = gamma * xhat + beta
      # out = gamma * (x-sMean)/torch.sqrt(sVar+eps) + beta
      # dout/dxhat * dxhat/dx
      # gamma * 
      # z = x - sMean
      # dxhat/dx = dxhat/dz * dz/dx
      # 


      # TODO: Cache
      cache = (x, xhat, sMean, sVar, eps, gamma, beta)
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    elif mode == 'test':
  ...
  def backward(dout, cache):
    ...
    # page 3 of https://arxiv.org/pdf/1502.03167.pdf
    # 
    dxhat = dout * gamma
  ...
  def backward_alt(dout, cache):
    ...
    # https://www.ryli.design/bnbackpass :)
