...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        ############################################################################################
        # Replace "pass" statement with your code
        x = torch.flatten(x, 1) # (N, H*W)
        # torch.cat((x, c), dim=1).shape == (N, H*W+C)
        x = self.encoder(torch.cat((x, c), dim=1))
...
def reparametrize(mu, logvar):
    ...
    # Convert logvar into std deviation sigma
...
def loss_function(x_hat, x, mu, logvar):
    ...
    dkl = -0.5 * (torch.sum(1 + logvar - torch.square(mu) - torch.exp(logvar)))
